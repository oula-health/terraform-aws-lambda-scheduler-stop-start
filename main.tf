...
resource "aws_lambda_function" "this" {
  ...
  environment {
    variables = {
      ...
      SCHEDULER_SCHEDULE              = tostring(var.scheduler_schedule)
      SCHEDULER_SCHEDULE_NAMES        = jsonencode(var.scheduler_schedule_names)
      ELASTICACHE_VALKEY_SCHEDULE     = tostring(var.elasticache_valkey_schedule)
      ELASTICACHE_VALKEY_REPLICATION_GROUPS_TO_DELETE = jsonencode(var.elasticache_valkey_replication_groups_to_delete)
      ELASTICACHE_VALKEY_REPLICATION_GROUPS_TO_CREATE = jsonencode(var.elasticache_valkey_replication_groups_to_create)
      ELB_SCHEDULE                    = tostring(var.elb_schedule)
      ELB_TO_DELETE                   = jsonencode(var.elb_to_delete)
      ELB_TO_CREATE                   = jsonencode(var.elb_to_create)
